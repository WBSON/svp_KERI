...
class ChromaBattSim(object):
    ...
    def set_init_soc(self, soc):
        #self.cmd("BATTery:PARA 1")
        self.cmd("BATTery:INITial 0")     
        #self.cmd("BATTery:INITial:VOLT 39")
    ...
    def set_volt_alarm(self, low, high):
        ...
        '''
        self.cmd("BATTery:BOH 110")
        self.cmd("BATTery:BOL -20")	
        self.cmd("BATTery:BVH 110" )     
        self.cmd("BATTery:VH 100")
        self.cmd("BATTery:VL 1")
        '''


    def set_volt_protection(self, low, high):
        self.cmd("BATTery:VOLP %.2f" % low)     
        self.cmd("BATTery:VOH %.2f" % high)     
        
        #self.cmd("BATTery:VOH 60")			# High Voltage Protection
        #self.cmd("BATTery:VOLP 0.01")			# Low Voltage Protection
        
        self.cmd("BATTery:BOH 110")         # high soc protection
        self.cmd("BATTery:BOL -20")         # low soc protection
        
        #self.cmd("BATTery:BVH 51")			# High Voltage Alarm
        #self.cmd("BATTery:BVH 110")
        #self.cmd("BATTery:BVL 1")				# Low Voltage Alarm (V)
